...
## Quick Rules
- Every route = folder in `src/app/` with `page.tsx` — no `page.tsx` means no public route
- Use `layout.tsx` for shared UI that persists across navigations (does NOT re-render)
- Use `loading.tsx` for instant loading states (wraps page in Suspense automatically)
- `error.tsx` must be `'use client'` — it's a scoped error boundary per segment
- Dynamic routes: `[param]` for single, `[...slug]` for catch-all, `[[...slug]]` for optional catch-all
- Route groups `(name)` organize routes without affecting the URL path
- Parallel routes `@slot` render multiple pages in the same layout with independent loading/error states
